    ...
    op.execute(
        ...
        CREATE TABLE sources (
            ...
            source_type VARCHAR(20) NOT NULL
                CHECK (source_type IN ('airline', 'ota', 'reference')),
            base_url TEXT,
        ...
        CREATE TABLE routes (
            ...
            route_key VARCHAR(7) GENERATED ALWAYS AS (
                origin || '-' || destination
            ) STORED UNIQUE,
            active BOOLEAN NOT NULL DEFAULT TRUE,
        ...
        CREATE TABLE fare_observations (
            ...
            user_development_fee NUMERIC(12, 2) NOT NULL DEFAULT 0
                CHECK (user_development_fee >= 0),
            ...
            availability VARCHAR(20) NOT NULL
                CHECK (availability IN ('available', 'sold_out', 'unknown')),
        ...
        CREATE TABLE index_runs (
            ...
            status VARCHAR(20) NOT NULL
                CHECK (status IN ('draft', 'published', 'revised', 'rejected')),
            value NUMERIC(14, 6),
...
def downgrade() -> None:
    op.execute(
        "DROP TABLE IF EXISTS index_runs, fare_observations, collection_runs, "
        "route_basket_members, route_baskets, routes, airlines, airports, sources CASCADE"
    )
